...
const i18n = {
    fr: {
        ...
        ui_reward_init: "L'Initiative (Vous jouez en premier)", ui_reward_shortcut: "Le Raccourci (+15 Lieues d'avance, l'ennemi joue en premier)",
    ...
    en: {
        ...
        rules_text: `<div style="font-family: 'Merriweather', serif; font-size: 14px; line-height: 1.6;"><h3 style="color:var(--gold); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">⚔️ THE GOAL</h3><p style="margin-top: 0;">Win 2 rounds by traveling 80 Leagues before your opponent.</p><h3 style="color:var(--gold); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">🎲 THE DICE</h3><ul style="margin-top: 0; padding-left: 20px; list-style-type: none;"><li><b style="color:var(--gold);">[ 6 ] Triumph</b>: 10 points.</li><li><b style="color:#5dade2;">[ 4, 5 ] Advance</b>: 4 or 5 points.</li><li><b style="color:#888;">[ 2, 3 ] Neutral</b>: No points.</li><li><b style="color:var(--blood);">[ 1 ] Ambush</b>: Deadly threat. Locks the die.</li></ul><h3 style="color:var(--gold); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">💀 ROUT & DEAD END</h3><p style="margin-top: 0;"><b>Rout:</b> Two "1"s. You lose your turn, points, and <b style="color:var(--gold);">1 Life</b>.</p><p><b>🛑 DEAD END:</b> 0 points rolled. Turn ends.</p><h3 style="color:var(--gold); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">⭐ HOPE (You)</h3><p style="margin-top: 0;">Gain +1 Hope per "1" rolled, or +2 by sacrificing a "6" at camp. Spend it to survive:</p><ul style="margin-top: 0; padding-left: 20px;"><li><b>Purify (-2)</b>: Click a "1" to cancel it and reroll.</li><li><b>Compass (-3)</b>: Reroll a Dead End.</li><li><b>A Elbereth (Empties Hope)</b>: Instantly blocks an enemy attack (Min. 3 Hope).</li></ul><p style="color: #5dade2; font-style: italic;">⚠️ Only 1 Hope Action allowed per turn!</p><h3 style="color:var(--blood); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">🔥 HATE (Enemy)</h3><p style="margin-top: 0;">The enemy gains Hate every turn or if you score \${SEUIL_HAINE}+ Leagues at once. If high enough, they launch a Malice attack. If cornered and losing the race, they might attempt a <b>Desperate Strike</b> to sabotage you even without Hate, risking exhaustion and the loss of Leagues or a Life.</p><h3 style="color:var(--corruption); font-family: 'Oswald', sans-serif; margin-bottom: 5px;">🌑 THE SHADOW (Corruption)</h3><p style="margin-top: 0;">Corrupting or devouring an enemy die gives you Shadow. Beware: <b>the higher your Shadow, the more your future rolls are corrupted</b> (fewer chances to roll 4, 5, and 6). Beyond 3 Shadow, each use might kill you instantly!</p></div>`,
...
function evaluateEnemyRoll(rolledIndices) {
    ...
    if (targetIdx !== -1) { gameState.currentEnemyRolledIndices = rolledIndices; gameState.currentEnemyTargetIdx = targetIdx; document.getElementById(`die-${targetIdx}`).style.boxShadow = "0 0 25px var(--corruption)"; let btnText = maxVal === 6 ? t('btn_corrupt') : t('btn_devour'); let statusText = maxVal === 6 ? t('status_shadow_6') : t('status_shadow_other', {val: maxVal}); updateStatus(statusText, "var(--corruption)"); document.getElementById('turn-controls').innerHTML = `<button onclick="corruptEnemyDie()" style="background: var(--corruption); color: #fff; border-color: var(--corruption);">${btnText}</button><button onclick="ignoreEnemyDie()" style="border-color: #555; color: #888;">${t('btn_ignore')}</button>`; return; }
...
function resolveRoundWinner(winner) {
    ...
    if (winner === 'hero') {
        ...
        } else { updateStaticUI(); document.getElementById('reward-modal').style.display = 'flex'; }
    } else {
